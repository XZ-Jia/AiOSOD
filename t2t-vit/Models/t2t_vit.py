...
class T2T_module(nn.Module):
    ...
    def forward(self, x, fea_4 , fea_8):
        # step0: soft split
        ...
        x_1_4 = self.attention1(x+fea_4)


        B, new_HW, C = x_1_4.shape
        x_1_4 = x_1_4.transpose(1,2).contiguous().reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))
        # iteration1: soft split
        x = self.soft_split1(x_1_4).transpose(1, 2)

        # iteration2: restricturization/reconstruction
        x_1_8 = self.attention2(x+fea_8)
        B, new_HW, C = x_1_8.shape
        x_1_8 = x_1_8.transpose(1, 2).contiguous().reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))
        # iteration2: soft split
        x = self.soft_split2(x_1_8).transpose(1, 2)
...
class T2T_ViT(nn.Module):
    ...
    def forward_features(self, x, fea_4, fea_8, fea_16):
        B = x.shape[0]
        x, x_1_8, x_1_4 = self.tokens_to_token(x, fea_4 , fea_8)
    ...
    def forward(self, x, fea_4 = 0, fea_8 = 0, fea_16 = 0):
        x, x_1_8, x_1_4 = self.forward_features(x, fea_4, fea_8, fea_16)
...
def T2t_vit_10(pretrained=False, **kwargs): # adopt performer for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 256 ** -0.5)
    model = T2T_ViT(tokens_type='performer', embed_dim=256, depth=10, num_heads=4, mlp_ratio=2.)
...
def T2t_vit_12(pretrained=False, **kwargs): # adopt performer for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 256 ** -0.5)
    model = T2T_ViT(tokens_type='performer', embed_dim=256, depth=12, num_heads=4, mlp_ratio=2.)
